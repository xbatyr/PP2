...
class SnakeGame:
    ...
    def make_obstacles(self):
        # add walls after level 2
        if self.level < 3:
            self.obstacles = set()
            return

        head = self.snake[0]
        # keep safe space near head
        safe = {
            head,
            (head[0] + 1, head[1]),
            (head[0] - 1, head[1]),
